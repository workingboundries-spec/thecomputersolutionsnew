...
// Every toggle defaults to true so existing call sites are unaffected.
...
export function QuotationPreview({
  ...
  // Determine if the totals block has anything to show
  const hasTotals = showSubtotal || (showDiscount && q.discount > 0) || (showGst && Number(q.gst_percent) > 0) || showGrandTotal;

  // Build visible columns list so we can span correctly
  const cols = [
    { id: "num",   show: true },
    { id: "desc",  show: true },
    { id: "qty",   show: true },
    { id: "price", show: showPriceCol },
    { id: "disc",  show: showDiscCol },
    { id: "total", show: showRowTotal },
  ];
  ...
  return (
    ...
    >
      ...
      {/* ── Quote meta ─────────────────────────────────────────────────── */}
      {(showQuoteNo || showDate || showValidTill) && (
        <div style={{ padding: "20px 28px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "1px solid #e5e7eb" }}>
          ...
          <div style={{ textAlign: "right", fontSize: 12, color: b.font, lineHeight: 1.6 }}>
            {showQuoteNo && <div><span style={{ opacity: 0.6 }}>Quote No: </span><strong>{q.quote_no}</strong></div>}
            {showDate    && <div><span style={{ opacity: 0.6 }}>Date: </span>{formatDate(q.created_at || new Date().toISOString())}</div>}
            {showValidTill && q.validity_date && <div><span style={{ opacity: 0.6 }}>Valid Till: </span>{formatDate(q.validity_date)}</div>}
          </div>
        </div>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: b.primary, color: onDarkText }}>
              ...
              {showPriceCol  && <th style={{ ...thStyle, width: 90,  textAlign: "right" }}>Price</th>}
              {showDiscCol   && <th style={{ ...thStyle, width: 60,  textAlign: "right" }}>Disc</th>}
              {showRowTotal  && <th style={{ ...thStyle, width: 100, textAlign: "right" }}>Total</th>}
            </tr>
          </thead>
          <tbody>
            {q.items.map((it, i) => {
              ...
              return (
                <tr key={i} style={{ background: i % 2 === 0 ? "#ffffff" : altRowBg }}>
                  ...
                  {showPriceCol && <td style={{ ...tdStyle, textAlign: "right", color: b.font }}>{formatINR(it.price)}</td>}
                  {showDiscCol  && <td style={{ ...tdStyle, textAlign: "right", color: b.font }}>{it.discount_pct || 0}%</td>}
                  {showRowTotal && <td style={{ ...tdStyle, textAlign: "right", fontWeight: 700, color: b.font }}>{formatINR(total)}</td>}
                </tr>
              );
            })}
          </tbody>
      ...
      {hasTotals && (
        <div style={{ padding: "16px 28px", display: "flex", justifyContent: "flex-end" }}>
          <div style={{ width: 300, fontSize: 13, color: b.font }}>
            {showSubtotal  && <Row k="Subtotal" v={formatINR(q.subtotal)} />}
            {showDiscount  && q.discount > 0 && <Row k="Discount" v={`- ${formatINR(q.discount)}`} />}
            {showGst       && Number(q.gst_percent) > 0 && <Row k={`GST (${q.gst_percent}%)`} v={formatINR(q.gst_amount)} />}
            {showGrandTotal && (
      ...
      {/* ── Terms / Notes ──────────────────────────────────────────────── */}
      {showTerms && ((showNotes && q.notes) || q.terms) && (
